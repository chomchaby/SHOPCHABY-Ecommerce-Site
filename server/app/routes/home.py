...
@app.route('/api/products/category=<category_name>/maxprice=<max_price>/minprice=<min_price>/sort=<sort>', methods = ['GET'])

def home_products(category_name, max_price, min_price, sort):
    print('Received GET request for /api/home/products/category/')
    print(category_name, max_price, min_price, sort)
    ...
    for pro in candidate_products:
        ...
        if max_price != 'undefined' and price_min>int(max_price): continue
        if min_price != 'undefined' and price_max<int(min_price): continue
